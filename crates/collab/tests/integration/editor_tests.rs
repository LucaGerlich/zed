...
async fn test_lsp_document_links(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    ...
    assert_eq!(
        ...
        "Host's editor should resolve its visible link locally"
    );
    ...
    assert_eq!(
        2,
        document_link_requests.load(atomic::Ordering::Acquire),
        "Guest's proto fetch should trigger a fresh documentLink LSP request on the host"
    );
    ...
    let full_range = guest_buffer.read_with(cx_b, |buffer, _| {
    ...
    project_b
        .update(cx_b, |project, cx| {
            project.lsp_store().update(cx, |lsp_store, cx| {
                lsp_store.resolve_document_links(&guest_buffer, &[full_range], cx)
            })
        })
        .await;
    executor.run_until_parked();
    ...
    assert_eq!(
        2,
        resolve_requests.load(atomic::Ordering::Acquire),
        "Guest's resolve should be forwarded over proto and run on the host"
    );
    ...
    let navigated = editor_b
        .update_in(cx_b, |editor, window, cx| {
            let snapshot = guest_buffer.read(cx).snapshot();
            let link = editor
                .document_link_at(buffer_id, &click_anchor, &snapshot)
                .expect("cached document link should cover the click anchor");
            let target = link.target.as_deref().expect("link should be resolved");
            let hover_link =
                editor::hover_links::document_link_target_to_hover_link(target, link.server_id);
